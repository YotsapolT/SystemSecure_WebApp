...
app.get("/", function (request, response) {
  //gen CSRFToken which is send between server and client 
  //(different from cookie which is only stored in local client and auto fill with request)
  request.session.csrf = request.csrfToken()
...
app.post("/auth", function (request, response) {
  ...
  if (username && password && request.session.csrf == request.body._csrf) {
  ...
  }else if(request.session.csrf !== request.body._csrf){
...
app.post("/registerEmail", function (request, response) {
  ...
  if (email && request.session.csrf == request.body._csrf) {
    ...
    response.render('home', { success: 'success', username: username, csrfToken: request.session.csrf })
  }else if(request.session.csrf !== request.body._csrf){
...
app.get("/home", function (request, response) {
  ...
  if (request.session.loggedin) {
    // Logged in
    response.render("home", { username: request.session.username, success: '', csrfToken: request.session.csrf });
  } else {
    // Not logged in
